...
var checkLogin = (req, res) => {
  if (req.session.sessionId) {
    return res
      .status(200)
      .send({ loged: true, username: req.session.username });
  ...
  if (usr && psd)
    return users
      ...
      .catch((e) => {
        ...
        return res
          .status(500)
          .send({ ok: false, messsage: "Internal Server Error" });
      });
  else
    return res
      .status(200)
      .send({ ok: false, message: "Please fill out all the necessary forms" });
};
...
socketserver.on("connection", (sock, req) => {
  ...
  sock.on("message", (data) => {
    ...
    if (jdata.command == "add_chat") {
      if (
        jdata.chat_name !== "" &&
        jdata.chat_name !== null &&
        jdata.chat_name !== undefined
      ) {
        if (/^[A-Za-z0-9\ ]+$/g.test(jdata.chat_name))
          ...
          send(user, {
            ok: false,
            message:
              "chat name can only consist of lower case, upper case, numbers and spaces",
            command: "error",
          });
      }
      ...
      if (
        jdata.chat_id !== "" &&
        jdata.chat_id !== null &&
        jdata.chat_id !== undefined &&
        /^[0-9]+$/g.test(jdata.chat_id)
      ) {
        message_handler
          .get_usernames(user, jdata)
          .then((res) => {
            jdata.sender = user;
      ...
      if (
        ...
        jdata.person != "" &&
        /^[0-9]+$/g.test(jdata.chat_id) &&
        validateUsername(jdata.person) &&
        jdata.person !== null &&
        jdata.chat_id !== null &&
        jdata.person !== undefined &&
        jdata.chat_id !== undefined
      ) {
